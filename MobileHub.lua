--[[
    MobileHub - Auto Farm para Blox Fruits (Roblox)
    Desenvolvido por: SeuNick
    Inspirado no estilo visual do Redz, mas 100% autoral
]]
...
local EnemyBox = Instance.new("TextBox")
...
Title.Text = "MobileHub - Auto Farm"
...
EnemyBox.Parent = FarmTab
EnemyBox.Position = UDim2.new(0.1, 0, 0.4, 0)
EnemyBox.Size = UDim2.new(0.8, 0, 0.2, 0)
EnemyBox.PlaceholderText = "Nome do Inimigo (ex: Bandit)"
EnemyBox.BackgroundColor3 = Color3.fromRGB(60, 0, 90)
EnemyBox.TextColor3 = Color3.fromRGB(255, 255, 255)
EnemyBox.Font = Enum.Font.Gotham
EnemyBox.TextSize = 14
...
game:GetService("RunService").RenderStepped:Connect(function()
    if farming and EnemyBox.Text ~= "" then
        for _, v in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
            if v:FindFirstChild("HumanoidRootPart") and v.Name:lower():find(EnemyBox.Text:lower()) and v:FindFirstChild("Humanoid") and v.Humanoid.Health > 0 then
                repeat
                    pcall(function()
                    ...
                    task.wait(0.2)
                until not v or not v.Parent or v.Humanoid.Health <= 0 or not farming
            end
        end
    end
end)
